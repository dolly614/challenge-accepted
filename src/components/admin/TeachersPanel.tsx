...
import {
  balance, listFraud, listTeachers, listWithdrawals, setWithdrawalStatus, updateTeacher, seedDemoIfEmpty,
} from "@/lib/teachers";
...
export function TeachersPanel() {
  ...
  const teachers = useMemo(() => listTeachers(), [tick]);
  const wds = useMemo(() => listWithdrawals(), [tick]);
  const fraud = useMemo(() => listFraud(), [tick]);

  const total = teachers.length;
  const approved = teachers.filter(t => t.status === "approved").length;
  const pending = teachers.filter(t => t.status === "pending").length;
  const totalPaid = wds.filter(w => w.status === "paid").reduce((a, w) => a + w.amount, 0);
              ...
              {teachers.map(t => {
                ...
                return (
                  <tr key={t.id} className="border-t border-border">
                    ...
                    <td className="px-2 py-2">{t.fraudScore > 40 ? <span className="text-destructive font-semibold">{t.fraudScore}</span> : t.fraudScore}</td>
                    <td className="px-2 py-2 text-right">
                      <div className="inline-flex gap-1">
                        {t.status !== "approved" && <Btn onClick={() => { updateTeacher(t.id, { status: "approved" }); refresh(); }}>Approve</Btn>}
                        {t.status !== "rejected" && <Btn tone="warn" onClick={() => { updateTeacher(t.id, { status: "rejected" }); refresh(); }}>Reject</Btn>}
                        {t.status === "approved" && <Btn tone="warn" onClick={() => { updateTeacher(t.id, { status: "suspended" }); refresh(); }}>Suspend</Btn>}
                      </div>
                    </td>
                  </tr>
                );
              ...
              {wds.map(w => {
                ...
                return (
                  <tr key={w.id} className="border-t border-border">
                    <td className="px-2 py-2 text-xs">{new Date(w.createdAt).toLocaleDateString("en-IN")}</td>
                    <td className="px-2 py-2">{t?.name || w.teacherId}</td>
                    ...
                    <td className="px-2 py-2 text-right">
                      <div className="inline-flex gap-1">
                        {w.status === "requested" && <Btn onClick={() => { setWithdrawalStatus(w.id, "approved"); refresh(); }}>Approve</Btn>}
                        {w.status !== "paid" && w.status !== "rejected" && <Btn onClick={() => { setWithdrawalStatus(w.id, "paid"); refresh(); }}>Mark Paid</Btn>}
                        {w.status !== "rejected" && w.status !== "paid" && <Btn tone="warn" onClick={() => { setWithdrawalStatus(w.id, "rejected"); refresh(); }}>Reject</Btn>}
                      </div>
                    </td>
                  </tr>
                );
              ...
              return (
                <li key={f.id} className="rounded-lg border border-border bg-background p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-xs text-muted-foreground">{new Date(f.createdAt).toLocaleString("en-IN")}</div>
                  </div>
                  <div className="text-xs text-muted-foreground">{t?.name || f.teacherId} — {f.detail}</div>
                </li>
              );
            })}
          </ul>
