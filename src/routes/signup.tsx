...
import { UserPlus, Mail, Lock, User as UserIcon, GraduationCap } from "lucide-react";

export const Route = createFileRoute("/signup")({
  head: () => ({ meta: [{ title: "Sign up — Uyanix" }] }),
      ...
      <section className="mx-auto max-w-md px-4 py-12 sm:px-6">
        <div className="rounded-3xl border border-border bg-card p-8 shadow-card">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-content-center rounded-xl bg-primary/10 text-primary"><UserPlus className="h-5 w-5"/></div>
            <div>
              <h1 className="text-2xl font-bold">Create account</h1>
              <p className="text-sm text-muted-foreground">Start your 30 days challenge</p>
            </div>
          </div>
          ...
          <form onSubmit={onSubmit} className="space-y-3">
            <Field icon={<UserIcon className="h-4 w-4 text-muted-foreground"/>} label="Full name">
              <input required value={fullName} onChange={e=>setFullName(e.target.value)}
                className="h-11 w-full bg-transparent text-sm outline-none" placeholder="Your name"/>
            </Field>
            <Field icon={<GraduationCap className="h-4 w-4 text-muted-foreground"/>} label="Class">
            ...
            <Field icon={<Lock className="h-4 w-4 text-muted-foreground"/>} label="Password (min 6)">
